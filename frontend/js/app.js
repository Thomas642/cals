...
function setupHistory() {
  ...
  navBtn.addEventListener('click', async () => {
    try {
      await loadScript('/js/history.js?v=26');
...
// ── Zones ─────────────────────────────────────────────────────────────────────
// `zones` and loadZones() stay here because they're used by other modules
// at boot (map markers, ETA calculation in renderMemberList).
// setupZones / renderZoneList are lazy-loaded from /js/zones.js on first
// navZones click.
let zones = [];
...
function setupZones() {
  const navBtn = document.getElementById('navZones');
  if (!navBtn) return;
  navBtn.addEventListener('click', async () => {
    try {
      await loadScript('/js/zones.js?v=26');
      setupZones();      // zones.js redefines the global with the real impl
      navBtn.click();    // re-fire the click to open the panel
    } catch (err) {
      console.error('[zones] lazy-load failed:', err);
      showToast('Erreur', 'Impossible de charger le module Zones');
    }
  }, { once: true });
}
...
function setupAdmin() {
  ...
  const onFirstClick = async () => {
    try {
      await loadScript('/js/admin.js?v=26');
...
async function setupChatOpen() {
  if (!_chatScriptPromise) _chatScriptPromise = loadScript('/js/chat.js?v=26');
